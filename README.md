...
#### Step 3: Create Database at phpMyAdmin named `comment_system_in_laravel` and setup .env file in your root directory 
...
DB_HOST=127.0.0.1
DB_PORT=3306
DB_DATABASE=comment_system_in_laravel
DB_USERNAME=root
DB_PASSWORD=
```
...
- we are going to create comment system from scratch. so we have to create migration for "posts" and "comments" table using Laravel 7 php artisan command, so first fire bellow command:
